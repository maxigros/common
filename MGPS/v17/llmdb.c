...
//		}
	...
	{	
		...
		if ((!session_already_exists) && (!spi_no_space))
		{
			ccs |= (0x00 << 4);
...
//   6    //идет запись сессии
	...
	{	
        ...
        {
            page_num_read = spi_block_num /22 + 1;
	...
	if(cmd == CMD_SPI_FSPACE)
	{
		...
		llmdb_ans[4] = (spi_free_space >> 24) & 0xFF;
		llmdb_ans[3] = (spi_free_space >> 16) & 0xFF;
		llmdb_ans[2] = (spi_free_space >> 8) & 0xFF;
		llmdb_ans[1] = spi_free_space & 0xFF;
	}
	...
	{	
        ...
        if (spi_sec_num < 1 || spi_sec_num > 22)
            ccs |= (0x0e << 4);
        else
        {
            ccs |= (0x03 << 4);
