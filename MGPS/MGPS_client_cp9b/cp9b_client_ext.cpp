...
cp9b_client_ext::cp9b_client_ext(QObject *parent) : cp9b_client(parent)
{
    mode_none = true;
...
void cp9b_client_ext::initiate_mode(mode_task *task)
{
...
void cp9b_client_ext::handle_received_package(char *buf, int size)
{
        ...
        if (!mode_none)
        {
            if (mode_sessions_rec)
            {
                ...
                case ((((CMD_BINR_START >> 3) + CMD_BINR_START) & 0x0F) | (0x00 << 4)):{
                    ...
                    timer_repeat.start(LINE_TIMEOUT);
            ...
            if (mode_flash_download)
            {
                ...
                case (((CMD_FLASH_READ_BLOCK >> 3) + CMD_FLASH_READ_BLOCK) & 0x0F) | (0x03 << 4):
                    ...
                    emit response(key, NULL, buf, size);
...
void cp9b_client_ext::start_sessions_rec(int *data)
{
    mode_none = false;
    mode_flash_download = false;
...
void cp9b_client_ext::start_session_download(QString session_name_str)
{
    mode_none = false;
    mode_flash_download = false;
    mode_sessions_rec = false;
...
void cp9b_client_ext::start_flash_download(int *data)
{
    mode_none = false;
...
void cp9b_client_ext::timers_stop()
{
    timer_repeat.stop();
...
void cp9b_client_ext::timer_repeat_timeout()
{
    cmd_handler(next_message);
    timer_repeat.start(LINE_TIMEOUT);
}

void cp9b_client_ext::timer_session_rec_duration_timeout()
{
    cmd_handler(next_message);
    timer_session_rec_duration.stop();
    timer_repeat.start(LINE_TIMEOUT);
...
void cp9b_client_ext::timer_flash_status_timeout()
{
...
bool cp9b_client_ext::check_contents(char *data)
{
    bool ok = false;
    if (data[0] == 'M' &&
        data[1] == 'G' &&
