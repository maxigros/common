...
void serial_prefix::extract_packs(QByteArray bb, int* dle_pos)
{
    /*
    // обрезаем справа
...
void serial_prefix::send_to_COM(char *data, int data_size)
{
    ...
    char* data_to_send = new char[prefix_size + 2 + pack_size + pack_size / 3];
    for (int i = 0; i < prefix_size; i++)
        data_to_send[i] = prefix[i];

    // package size
    data_to_send[prefix_size] = data_size & 0xFF;
    data_to_send[prefix_size] = (data_size >> 8) & 0xFF;
    ...
    serial.write((const char*)data_to_send, pos + data_size);
